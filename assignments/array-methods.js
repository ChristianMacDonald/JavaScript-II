...
ticketPriceTotal = runners.reduce((ticketPriceTotal, runner) => {
...
// Problem 1
// Jabbersphere has been feeding their workers performance enhancing drugs. Their runners have been banned. Create an array of all the runners who are banned.
let bannedRunners = [];
bannedRunners = runners.filter(runner => {
    return runner.company_name === "Jabbersphere";
});
console.log(bannedRunners);

// Problem 2
// The event director has run out of Ms for the running bibs. Find out how many runners have M in their names.
let runnersWithMNames = [];
runnersWithMNames = runners.filter(runner => {
    return runner.first_name.includes("M") || runner.first_name.includes("m") || runner.last_name.includes("M") || runner.last_name.includes("m");
});
console.log(runnersWithMNames.length);

// Problem 3
// The event director is making a poster for the sponsors. Create an array with all the company names.
companyNames = [];
runners.forEach(runner => {
